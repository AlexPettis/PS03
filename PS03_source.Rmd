...
The listings variable is the total active listings.
...
```{r}
# Type your code and comments inside the code chunk
txhousing <- select(txhousing, -inventory)
head(txhousing)
```
...
```{r}
# Type your code and comments inside the code chunk
txhousing %>% filter(year == 2012 | 2013, city == "Dallas") -> dallas_sub
```
...
```{r}
# Type your code and comments inside the code chunk
dallas_sub %>% group_by(month) %>% summarize(mean_prct_sold = mean(prct_sold)) -> dallas_summary
dallas_summary
```
...
```{r}
# Type your code and comments inside the code chunk
dallas_summary  %>% arrange(desc(mean_prct_sold))
```
...
The code does everything that we did in the previous questions.
...
```{r}
# Type your code and comments inside the code chunk
txhousing %>% filter(year == 2015, month == 1) %>% arrange(listings) %>% head(n = 3) -> question9
question9
```

<div id="answer">

San Marcos is the city that had the fewest houses listed for sale in January 2015.
...
txhousing %>% 
  ...
  arrange(desc(top_sales)) -> question10
question10
```

<div id="answer">

In 2012, **`r month.name[question10$month[1]]`** had the most houses (`r comma(question10$top_sales[1])`) sold in Texas.
</div> 
...
```{r}
# Type your code and comments inside the code chunk
txhousing %>% filter(city == "Austin" | city == "Dallas") %>% filter(year == 2000 | year == 2001) %>% group_by(city) %>% summarize(total_sales = sum(sales))
```
